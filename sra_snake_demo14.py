#!/usr/bin/env python3
"""
SRA: SELF-SUPERVISED NEURO-SYMBOLIC AGENT (FINAL + SAVE)
========================================================
Features:
...
6. Model Saving (Saves 'sra_brain.pth' for visualization)

Usage: python sra_snake_final.py
"""
...
CONFIG = {
    ...
    'ae_epochs': 200,          # High epoch count for sharp vision
    'ae_lr': 1e-3,
    'latent_dim': 16,
    ...
    'memory_capacity': 50000,  # Large memory buffer
    
    # Phase 3: PPO & Curiosity
    'ppo_updates': 2000,       # Long run duration
    'rollout_size': 512,
    'ppo_lr': 3e-4,
...
def main():
    ...
    print("\n[Phase 2] Living Kernel PPO Loop...")
    ...
    for update in range(1, CONFIG['ppo_updates'] + 1):
        ...
        for _ in range(CONFIG['rollout_size']):
            ...
            # Storage
            # FIX: Explicit detach for safety on all stored tensors
            states.append(z_curr.detach()) 
            actions.append(action.detach())
            logprobs.append(dist.log_prob(action).detach()) 
            rewards.append(intrinsic_reward * CONFIG['curiosity_weight'])
            dones.append(done)
        ...
        if update % 50 == 0:
            print(f"  Update {update}/{CONFIG['ppo_updates']}: Curiosity={avg_cur:.4f} | Memory Size={len(memory)}")

    save_plot(curiosity_history, "Curiosity Curve", "2_curiosity.png")
