#!/usr/bin/env python3
"""
SRA: REBOOT (HIGH ENTROPY MODE)
===============================
Tuned to break 'Mode Collapse' (always going Right).
- Higher Entropy (Forces random exploration)
- Lower Kernel Bias (Reduces reliance on bad early memories)
- Survival Bonus (Rewarding existence to prevent suicide)
"""
...
# ==========================
CONFIG = {
    'seed': 999, # New seed to break old patterns
    'device': 'cuda' if torch.cuda.is_available() else 'cpu',
    'output_dir': 'sra_reboot_output',
    ...
    'ae_epochs': 100, # Fast but sufficient
    'ae_lr': 1e-3,
    'latent_dim': 16,
    ...
    # PPO & Curiosity
    'ppo_updates': 3000,        # Longer run
    'rollout_size': 512,
    'ppo_lr': 3e-4,
    'curiosity_weight': 2.0,    # Boost curiosity
    'gamma': 0.99,
    'entropy_coef': 0.05,       # <--- CRITICAL FIX: Higher randomness
    
    # Living Kernel
    'living_kernel_strength': 0.1, # <--- FIX: Lower bias to prevent loops
...
print(f"--- SRA REBOOT INITIALIZED ON {device} ---")
...
class SnakeEnv:
    ...
    def reset(self):
        # Randomize start position to prevent "Right-Bias"
    ...
    def step(self, action):
        ...
        if nx < 0 or ny < 0 or nx >= self.size or ny >= self.size or (nx,ny) in self.snake:
            self.done = True
            return self._get_obs(), -1.0, True, {} # Pain

        self.snake.insert(0, (nx,ny))
        if self.food and (nx,ny) == self.food:
            self.place_food()
            reward = 0.5 # Small extrinsic reward for food
        else:
            self.snake.pop()
            reward = 0.01 # Survival Bonus (Keep moving!)
...
def main():
    ...
    print("\n[Phase 2] Living Kernel PPO Loop (Rebooted)...")
    ...
    for update in range(1, CONFIG['ppo_updates'] + 1):
        ...
        for _ in range(CONFIG['rollout_size']):
            ...
            # --- HYBRID REWARD ---
            total_reward = (intrinsic_reward * CONFIG['curiosity_weight']) + reward_env
