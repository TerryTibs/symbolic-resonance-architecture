...
# =============================================================================
# 1. UPGRADED HYPERPARAMETERS
# =============================================================================
CONFIG = {
    "grid_size": 12,
    "num_agents": 8,               # Increased for faster parallel learning
    "latent_dim": 64,              # Doubled for better memory capacity
    "bottleneck_dim": 16,          # Doubled for more complex feature extraction
    "total_steps": 15001,          # Increased steps for growth training
    ...
    "field_coupling": 0.90,        # Stronger global manifold influence
    "oracle_steps": 3000,          # Longer grounding period
    "base_entropy": 0.3,           # Less noise, more exploitation
    "bias_decay_end": 10000,       # Step where the "12.0 bias" reaches zero
...
# =============================================================================
# 2. PERFORMANCE MONITORING SUITE (Maintained)
# =============================================================================
class PerformanceMonitor:
    ...
    def log_status(self, step, succ, mort, envs, agents, telemetry):
        ...
        print(f" SPECTRUM   | Spectral Entropy: [{self.sparkline(weight_health)}] {np.mean(weight_health):.3f}/4.15") # Adjusted for 64 latent
        ...
        print(f" AGENT_0    | Body Length: {len(envs[0].snake)} | Cert: {telemetry[0]['ct']:.2f}")
        if step % 200 == 0:
            print("\n--- ENVIRONMENT RENDER ---")
            print(envs[0].render_ascii())

# =============================================================================
# 3. NEURAL ARCHITECTURE (Maintained & Scaled)
# =============================================================================
class AgentNetwork(nn.Module):
    def __init__(self, config):
        ...
        self.reset_count, self.death_streak, self.spectral_entropy = 0, 0, 4.15
...
# =============================================================================
# 4. ENVIRONMENT: GROWING SNAKE SIMULATOR
# =============================================================================
class SnakeEnv:
    ...
    def step(self, action):
        ...
        if hx < 0 or hy < 0 or hx >= self.size or hy >= self.size or (hx, hy) in self.snake:
            self.done = True; return self.get_obs(), -50.0, True, False, 0, old_d
        self.snake.insert(0, (hx, hy))
        new_d = abs(hx-self.food[0]) + abs(hy-self.food[1])
        # Eating logic (Growth!)
        if (hx, hy) == self.food:
            self.food = self.spawn_food()
            self.lag = 0; return self.get_obs(), 200.0, False, True, old_d-new_d, 0
        # Normal movement (Removed 5-segment limit)
        self.snake.pop()
...
# =============================================================================
# 5. TRAINING AND EXECUTION (Curriculum Evolution)
# =============================================================================
def train():
    ...
    for step in range(CONFIG["total_steps"]):
        # CALCULATE CURRICULUM BIAS: Starts at 12.0, decays to 0.0
        ...
        for i, agent in enumerate(agents):
            ...
            # Directional Prior (Decaying Bias)
            ...
            if ate: total_succ += 1; winners.append(i)
            if done: total_mort += 1
            ...
            # Smart Reward: reward moving toward food based on internal geometric vector
            ...
            step_logs.append({'target_gain': tg.item(), 'proprioception': sa.item(), 'ct': agent.last_cert})

        if len(winners) > 0: latent_field = 0.8 * latent_field + 0.2 * torch.cat(latent_stack, 0)[winners].mean(dim=0, keepdim=True).detach()
        if step % 100 == 0: monitor.log_status(step, total_succ, total_mort, envs, agents, step_logs)
