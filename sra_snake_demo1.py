#!/usr/bin/env python3
"""
SRA Snake – all patches integrated
...
# ------------------------------------------------------------------
# Imports – keep the imports in a single block
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Configuration – keep everything in a single dict
# ------------------------------------------------------------------
CONFIG = {
    ...
    # environment (the snake world)
    'env_size'            : 8,          # grid size
    'max_episode_steps'   : 5000,        # truncate episodes

    # AE pre‑training – collect a dataset of random states
    ...
    'memory_absolute_threshold': 0.012,
    'memory_relative_factor'   : 0.80,
...
# ------------------------------------------------------------------
# Helper functions – keep them compact
# ------------------------------------------------------------------
def ensure_dir(p):
    Path(p).mkdir(parents=True, exist_ok=True)

def flatten_obs(obs):
    """Return a 1‑D float32 vector from any common image layout."""
    a = np.array(obs, dtype=np.float32)
    # normalise channel first (3,H,W) or (H,W,3)
...
def obs_to_tensor(obs):
    """Wrap flatten_obs into a torch Tensor ready for autograd."""
    v = flatten_obs(obs)
...
class SnakeEnv:
    ...
    # Reset – start with a single snake cell in the centre
    def reset(self):
    ...
    # Copy / restore a whole state (for simulation)
    def copy_state(self):
    ...
    # Manhattan distance from snake head to the food
...
# ------------------------------------------------------------------
# Perceptual Core – a simple auto‑encoder
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Resonant Engine – optimises a latent vector at inference time
# ------------------------------------------------------------------
class ResonantEngine:
    ...
    def resonate(self, x, steps=None, lr=None):
        """Perform inference‑time optimisation on a latent vector z.
           Return (z_opt, final_loss)."""
        if steps is None:
            steps = CONFIG['resonance_steps']
        if lr is None:
            lr = CONFIG['resonance_lr']

        # initialise the latent vector
        with torch.no_grad():
            z0   = self.model.encoder(x)
            recon0= self.model.decoder(z0)
            z0_loss = float(self.criterion(recon0, x).item())

        # make it a parameter that will be optimised
        ...
        return z.detach(), float(loss.item()), z0_loss

# ------------------------------------------------------------------
# Cognitive memory – FIFO buffer with a gating condition
# ------------------------------------------------------------------
class CognitiveMemory:
    ...
    def add_event(self, latent_vector, loss, z0_loss=None):
        """Insert the vector into the memory if it is sufficiently coherent.
...
# ------------------------------------------------------------------
# AE pre‑training – collect a random dataset
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# AE training – standard MSE + scheduler + early stopping
# ------------------------------------------------------------------
def train_autoencoder(core, data,
                      epochs=20, batch_size=128, lr=1e-3,
                      device='cpu',
                      lr_step=5, lr_gamma=0.5,
...
# ------------------------------------------------------------------
# Adaptive symbol discovery – robust k‑means + elbow selection
# ------------------------------------------------------------------
def adaptive_symbol_discovery(memory_vectors, min_k=2, max_k=10):
    ...
    # ------------------------------------------------------------------
    # 1 – very few points → just cluster by exact duplicates
    ...
    # ------------------------------------------------------------------
    # 2 – not enough unique points → cluster by exact duplicates again
    ...
    # ------------------------------------------------------------------
    # 3 – run a short grid of k‑means & pick best elbow
...
# ------------------------------------------------------------------
# SRA action selection – epsilon‑greedy
# ------------------------------------------------------------------
def sra_choose_action(env, core, engine, epsilon=0.0, device='cpu'):
    ...
    for a in range(4):
        ...
        # shape‑reward → prefer states that shorten the distance to food
        ...
        # resonance – optimise the latent vector for this simulated state
        x       = obs_to_tensor(obs_sim).to(device)
        z_opt, loss_opt, z0_loss = engine.resonate(x)

        # total score: intrinsic + extrinsic + shaping + small tie‑breaker
        score   = -loss_opt + shaping
        if reward_sim > 0:
            score += CONFIG['reward_eat']
        if done_sim:
            score += CONFIG['reward_die']

        if score > best_score:
            best_score   = score
            best_action  = a
            best_details = {'sim_reward': reward_sim, 'loss_opt': loss_opt,
                             'z0_loss' : z0_loss, 'shaping': shaping}
...
def save_artifacts(core, centers, memory, labels, tag):
    ...
    # 2 – glyphs of the cluster centres
    with torch.no_grad():
...
def main():
    ...
    data   = collect_random_states(env, n_samples=CONFIG['collect_random_states'],
                                    max_steps=CONFIG['random_max_steps'])
    input_dim  = data.shape[1]
    core       = PerceptualCore(input_dim=input_dim,
    ...
    # Training episodes – populate the memory
    ...
    for ep in range(1, CONFIG['train_episodes']+1):
        ...
        for t in range(CONFIG['train_max_steps']):
            a, details = sra_choose_action(env, core, engine,
                                               epsilon=epsilon, device=device)
            obs, reward, done, _ = env.step(a)
            total_reward += reward
            steps          += 1

            # resonance & memory insertion
        ...
        print(f"[Train {ep}/{CONFIG['train_episodes']}] steps={steps} reward={total_reward}"
              f" memories={len(memory)} epsilon={epsilon:.3f} stored_this_ep={stored_count}")

        # debug prints
        if CONFIG['debug']:
            if len(z0_losses) > 0:
                print(f"  debug z0_loss mean={np.mean(z0_losses):.5f}"
                      f" min={np.min(z0_losses):.5f} z*_mean={np.mean(zstar_losses):.5f}"
                      f" z*_min={np.min(zstar_losses):.5f}")
        ...
        if len(memory) >= CONFIG['min_memory_for_clustering'] and (ep % CONFIG['save_every_n_episodes'] == 0):
            centers, labels, k = adaptive_symbol_discovery(memory.vectors,
                                                             min_k=CONFIG['min_symbols'],
                                                             max_k=CONFIG['max_symbols'])
