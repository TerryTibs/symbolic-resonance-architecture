...
class ConvAE(nn.Module):
    ...
    def decoder(self,z):
        h=self.decoder_fc(z)
        h=h.view(-1,32,CONFIG['grid_size'],CONFIG['grid_size'])
...
def select_action(env,resonator):
    ...
    for a in range(4):
        ...
        scores.append(-z.norm().item())  # simplistic epistemic score
        env.step((a+2)%4)  # revert move
...
def run_game():
    ...
    while True:
        ...
        a=select_action(env,resonator)
        obs,reward,done=env.step(a)
        mem.add_transition(env.snake[0],env.snake[0])
        plot_state(env,mem.G)
        if done:
            obs=env.reset()
            mem=MemoryGraph()
    plt.ioff()
