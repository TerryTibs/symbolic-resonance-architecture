...
CONFIG = {
    ...
    "oracle_steps": 1000,
    "base_entropy": 0.4,
    "model_path": "dual_path_agent.pth",
...
class TrainingMonitor:
    ...
    def log_status(self, step, succ, mort, envs, agents, telemetry):
        ...
        print(f" VISION     | SNR: {snr:.2f} [{self.sparkline(list(self.snr_history))}] | Target: {avg_tg*100:2.1f}% | Self: {avg_sa*100:2.1f}%")
...
class AgentNetwork(nn.Module):
    ...
    def calculate_centroids(self, logits, head_pos):
        ...
        # Coordinate selection via high-temperature spatial softmax
    ...
    def forward(self, x, head_pos):
        x_proc = x.clone()
        x_proc[:, 2, :, :] -= x_proc[:, 0, :, :] # Difference masking
        ...
        self_aware = (s_mask * x[:, 0, :, :].view(x.shape[0], -1)).sum()
        ...
        return z_i, z_e, self.self_predictor(z_i), z_v, target_v, self_v, target_gain, self_aware
...
# =============================================================================
def train():
    print(f"--- INITIALIZING RS-3 OMEGA ON {str(device).upper()} ---")
    ...
    for step in range(CONFIG["total_steps"]):
        ...
        for i, agent in enumerate(agents):
            ...
            # Action Space Shaping (Heuristic Compass)
            ...
            l_coord = 300.0 * F.mse_loss(target_v, t_rel) if step < CONFIG["oracle_steps"] else torch.tensor(0.0, device=device)
            ...
            # Corrected Telemetry Dictionary Keys
            telemetry.append({
                'target_gain': tg.item(), 
                'self_aware': sa.item(), 
                'rv': [tx, ty], 
                'sv': self_v[0].detach().cpu().numpy(), 
                'dr': d_raw, 
                'ct': agent.last_cert, 
                'det': target_v[0].detach().cpu().numpy()
            })
