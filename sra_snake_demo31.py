...
# -------------------------
# MULTI-STEP σ₃ PREDICTIONS USING LATENT SPACE
# -------------------------
def latent_sigma3_paths(env, resonator, lookahead=CONFIG['lookahead']):
    obs = env._get_obs()
    z = resonator.resonate(obs)
    paths = []

    # Decode multiple candidate directions
    directions = [[-1,0],[1,0],[0,-1],[0,1]]
    for d in directions:
        path = [env.snake[0].copy()]
        pos = env.snake[0].copy()
        for _ in range(lookahead):
            pos = [pos[0]+d[0], pos[1]+d[1]]
            if (not all(0<=c<CONFIG['grid_size'] for c in pos) or
                pos in path or pos in env.snake):
                break
            path.append(pos.copy())
        if len(path)>1:
            paths.append(path)
    # Epistemic pruning
    pruned = [p for p in paths if len(p)/lookahead >= CONFIG['prune_threshold']]
    return pruned
...
def select_action(env, resonator):
    paths = latent_sigma3_paths(env, resonator)
    if not paths:
        return np.random.randint(0,4), []
    # Choose path whose endpoint is closest to food
