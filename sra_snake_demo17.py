#!/usr/bin/env python3
"""
SRA: FOCUS MODE (FIXED)
=======================
Tuned to break the '0.25' flatline.
Fixed the ResonantEngine return value bug.
"""
...
print(f"--- SRA FOCUS MODE INITIALIZED ON {device} ---")
...
class SnakeEnv:
    ...
    def step(self, action):
        ...
        if nx < 0 or ny < 0 or nx >= self.size or ny >= self.size or (nx,ny) in self.snake:
            self.done = True
            return self._get_obs(), -2.0, True, {} # Stronger Pain
        ...
        if self.food and (nx,ny) == self.food:
            self.place_food()
            reward = 10.0 # MASSIVE REWARD
        else:
            self.snake.pop()
            reward = -0.01 # Mild hunger penalty
...
def main():
    ...
    print("\n[Phase 2] Living Kernel Focus Loop...")
    ...
    for update in range(1, CONFIG['ppo_updates'] + 1):
        ...
        for _ in range(CONFIG['rollout_size']):
            ...
            if random.random() < 0.1: 
                # This call now correctly unpacks two values
