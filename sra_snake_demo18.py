#!/usr/bin/env python3
"""
SRA: BIG BRAIN EDITION (High Capacity Model)
============================================
Upgraded Neural Architecture for complex reasoning.
- Deeper ConvAE (64 filters + 256 hidden units)
- Deeper Actor-Critic (2-layer reasoning head)
- "Focus Mode" Reward Tuning (+10 Food, -2 Death)

Usage: python sra_snake_big_brain.py
"""
...
CONFIG = {
    ...
    'output_dir': 'sra_big_brain_output',
    
    # World
    'grid_size': 100,
    'max_episode_steps': 2000,
    
    # Phase 1: Perception (Heavier Model needs more training)
    'pretrain_samples': 50,  # More data for the bigger brain
    'ae_epochs': 10,
    'ae_lr': 1e-3,
    'latent_dim': 32,          # Increased latent space for more detail
...
print(f"--- SRA BIG BRAIN INITIALIZED ON {device} ---")
...
class SnakeEnv:
    ...
    def step(self, action):
        ...
        if nx < 0 or ny < 0 or nx >= self.size or ny >= self.size or (nx,ny) in self.snake:
            self.done = True
            return self._get_obs(), -2.0, True, {} # Strong Pain
...
# ==========================
# MODULE 1: BIG PERCEPTION (Upgraded)
# ==========================
class ConvAE(nn.Module):
    def __init__(self, latent_dim):
        super().__init__()
        # Deeper Encoder: 32 -> 64 filters + Intermediate Dense Layer
        self.encoder = nn.Sequential(
            nn.Conv2d(3, 32, 3, padding=1), nn.ReLU(),
            nn.Conv2d(32, 64, 3, padding=1), nn.ReLU(),
            nn.Flatten(),
            nn.Linear(64 * CONFIG['grid_size']**2, 256), # Intermediate logic
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(256, 64 * CONFIG['grid_size']**2), nn.ReLU(),
            nn.Unflatten(1, (64, CONFIG['grid_size'], CONFIG['grid_size'])),
...
class ResonantEngine:
    ...
    def resonate(self, x_img, steps=None):
        ...
        with torch.no_grad(): z0 = self.model.encoder(x_img)
...
# ==========================
# MODULE 4: BIG BRAIN (Upgraded Actor)
# ==========================
class ActorCritic(nn.Module):
    def __init__(self, latent_dim, action_dim=4):
        super().__init__()
        # Deeper Reasoning Network (2 Layers)
...
def main():
    ...
    print("\n[Phase 1] Pretraining Eyes (Heavy)...")
    data = []
    print(f"  > Collecting {CONFIG['pretrain_samples']} samples...")
    ...
    print("\n[Phase 2] Living Kernel Loop (Big Brain)...")
    ...
    for update in range(1, CONFIG['ppo_updates'] + 1):
        ...
        for _ in range(CONFIG['rollout_size']):
            ...
            # Hybrid Reward
            intrinsic = F.mse_loss(pred_z, z_next.detach()).item()
