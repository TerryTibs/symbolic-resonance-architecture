#!/usr/bin/env python3
"""
SRA UNIFIED SNAKE KERNEL
Fully functional Neuro-Symbolic Snake with Perception, Resonance, Memory, Symbols, and PPO+Curiosity.
...
class ForwardPredictor(nn.Module):
    def __init__(self, latent_dim, action_dim=4):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(latent_dim+action_dim,64), nn.ReLU(), nn.Linear(64, latent_dim))
...
def main():
    ...
    for ep in range(CONFIG['ae_epochs']):
        ...
        if (ep+1)%10==0: print(f"  Epoch {ep+1}: Loss={epoch_loss/len(data):.5f}")

    # PHASE 2: CHILDHOOD
    print("\n[Phase2] Childhood: Resonant Exploration")
    for ep in range(1,CONFIG['childhood_episodes']+1):
        ...
        while True:
            ...
            if done: break
        if ep%5==0: print(f"  Ep {ep}: Reward={rewards:.1f} | Memory={len(memory.vectors)}")

    # PHASE3: LANGUAGE
    print("\n[Phase3] Symbol Discovery")
    mem_arr = memory.get_data()
    if len(mem_arr)<CONFIG['min_memory_clustering']:
        print("  ! Not enough memories. Using random")
    ...
    # PHASE4: ADULTHOOD
    print("\n[Phase4] PPO Training with Curiosity")
    symbol_emb_dim = 8
    ...
    for update in range(1, CONFIG['adult_updates']+1):
        states, actions, logprobs, rewards_list, dones, values = [],[],[],[],[],[]
        obs = env.reset(); ep_rew=0
        ...
        # --- PPO Update
        returns=[]; gae=0; next_val=0
        for i in reversed(range(len(rewards_list))):
            delta = rewards_list[i] + CONFIG['gamma']*next_val*(1-dones[i]) - values[i].item()
        ...
        b_logprobs = torch.stack(logprobs); b_returns = returns
        b_values = torch.cat(values); b_advantages = b_returns - b_values.squeeze()
        for _ in range(CONFIG['ppo_epochs']):
            indices = torch.randperm(len(b_states))
            for start in range(0, len(b_states), CONFIG['batch_size']):
                end = start+CONFIG['batch_size']; idx = indices[start:end]
                ...
                surr2 = torch.clamp(ratio,1-CONFIG['clip_ratio'],1+CONFIG['clip_ratio'])*b_advantages[idx]
        ...
        if update%5==0:
            avg_r = np.mean(history_rewards[-5:])
