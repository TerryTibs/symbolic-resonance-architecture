...
# =============================================================================
def train():
    print(f"--- RS-3 OMEGA (FIXED DATA KEYS) ON {str(device).upper()} ---")
    diag = Diagnostics()
    ...
    for step in range(CONFIG["total_steps"]):
        ...
        for i, agent in enumerate(agents):
            ...
            l_oracle = 300.0 * F.mse_loss(target_v, t_rel) if step < CONFIG["oracle_steps"] else torch.tensor(0.0, device=device)
            ...
            total_loss = l_rl + l_unity + l_oracle + l_ortho + CONFIG["self_awareness_weight"] * (1.0 - sa) + CONFIG["vision_gate_weight"] * (0.0 - tg)
        ...
        if step % 50 == 0:
            ...
            sys.stdout.write(f"\rStep {step:04d} | SNR: {telemetry[0]['target_gain']/max(0.01, telemetry[0]['self_awareness']):.2f} | Lag: {envs[0].lag:02d} | C: {telemetry[0]['ct']:.2f}")
            sys.stdout.flush()
