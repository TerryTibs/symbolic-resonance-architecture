...
# =============================================================================
# 1. SYSTEM HYPERPARAMETERS (Fixed & Synchronized)
# =============================================================================
CONFIG = {
    "grid_size": 48,
    "num_agents": 6,               
    ...
    "field_coupling": 0.95,        
    "oracle_steps": 15000,         
    "base_entropy": 0.3,            # Fixed missing key
    "bias_decay_end": 45000,       
...
class PerformanceMonitor:
    ...
    def log_status(self, step, succs, morts, envs, agents, telemetry):
        ...
        print(f"\n[STEP {step:04d}] " + "="*100)
        print(f" GLOBAL STATS | Total Succ: {sum(succs):5d} | Total Mort: {sum(morts):5d} | TPS: {step/max(0.1,elapsed):.1f}")
        print(f" VISION SNR   | [{self.sparkline(list(self.snr_history))}] {snr:.2f} | Oracle: {'ON' if step < CONFIG['oracle_steps'] else 'OFF'}")
        print("-" * 112)
        print(f" ID  | SUCC  | LAG | CERT | GAZE (X,   Y)   | OPTIMISM | SPECTRAL | STATUS")
        print("-" * 112)
        ...
        for i in range(CONFIG["num_agents"]):
            ...
            if t['val'] > 1.0: status = "TARGETED "

            print(f" A{i}  | {succs[i]:5d} | {e.lag:03d} | {t['ct']:.2f} | [{gx:+.2f}, {gy:+.2f}] | {t['val']:+.4f}  |  {a.spectral_entropy:.2f}    | {status}")

        if step % 500 == 0:
            print("\n--- SPATIAL PROJECTION (AGENT 0) ---")
...
class SnakeEnv:
    ...
    def step(self, action):
        if self.done: return self.get_obs(), 0.0, True, False, 0
        self.lag += 1
        if self.lag > self.starvation_limit:
            self.done = True; return self.get_obs(), -15.0, True, False, 0
        ...
        if hx < 0 or hy < 0 or hx >= self.size or hy >= self.size or (hx, hy) in self.snake:
            self.done = True; return self.get_obs(), -50.0, True, False, 0
        ...
        if (hx, hy) == self.food:
            ...
            self.lag = 0; return self.get_obs(), 200.0, False, True, old_d-new_d
        self.snake.pop()
        return self.get_obs(), -0.1, False, False, old_d-new_d
    def get_obs(self):
...
def train():
    ...
    for step in range(CONFIG["total_steps"]):
        ...
        for i, agent in enumerate(agents):
            ...
            n_obs, rew, done, ate, d_delta = envs[i].step(action)
            if ate: agent_succs[i] += 1; winners.append(i)
            if done: agent_morts[i] += 1
        ...
        if step % 100 == 0: monitor.log_status(step, agent_succs, agent_morts, envs, agents, step_logs)
