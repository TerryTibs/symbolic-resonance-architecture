...
# =============================================================================
# 1. SYSTEM CONFIGURATION (Optimized to break the 134-Success Plateau)
# =============================================================================
CONFIG = {
    "grid_size": 12,
    "num_agents": 1,
    ...
    "spectral_weight": 0.8,
    "vision_gate_weight": 150.0,    # INCREASED: Force focus on vision
    "alignment_weight": 40.0,
    "proprioceptive_weight": 35.0,
    "field_coupling": 0.40,        # DECREASED: Break the "Zombie" loop, allow autonomy
    "oracle_steps": 5000,
    "base_entropy": 0.3,
    "bias_decay_end": 30000,
...
# =============================================================================
# 4. ENVIRONMENT (With Brighter Food)
# =============================================================================
class SnakeEnv:
...
def train():
    ...
    obs_batch = [torch.tensor(e.reset(), dtype=torch.float32).unsqueeze(0).to(device) for e in envs]
    ...
    for step in range(CONFIG["total_steps"]):
        ...
        for i, agent in enumerate(agents):
            ...
            reward_t = torch.tensor([[rew + d_delta * 10]], device=device)
            adv = reward_t - value.detach()
            policy_loss = -dist.log_prob(torch.tensor([action], device=device)) * adv
            value_loss = F.mse_loss(value, reward_t)
            true_v = (torch.tensor([[envs[i].food[0], envs[i].food[1]]], dtype=torch.float32, device=device) - h_pos) / (CONFIG["grid_size"] / 2.0)
            ...
            l_ortho = CONFIG["spectral_weight"] * F.mse_loss(agent.path_int.weight @ agent.path_int.weight.t(), torch.eye(CONFIG["latent_dim"], device=device))
            l_ent = CONFIG["base_entropy"] * dist.entropy().mean()
            total_loss = policy_loss + value_loss + l_ent + l_unity + l_coord + l_ortho + CONFIG["proprioceptive_weight"]*(1.0-sa) + CONFIG["vision_gate_weight"]*(0.0-tg)
            ...
            obs_batch[i] = torch.tensor(envs[i].reset() if done else n_obs, dtype=torch.float32).unsqueeze(0).to(device)
            step_logs.append({'target_gain': tg.item(), 'proprioception': sa.item(), 'ct': 1.0-(dist.entropy().mean().item()/1.38), 'val': value.item(), 'gaze': [tx, ty]})
            latent_stack.append(z_v.detach())
        if len(winners) > 0: latent_field = 0.8 * latent_field + 0.2 * torch.cat(latent_stack, 0)[winners].mean(dim=0, keepdim=True)
        if step % 100 == 0:
            monitor.log_status(step, agent_succs, agent_morts, agent_starves, envs, agents, step_logs)
            if step % 1000 == 0: torch.save(agents[0].state_dict(), CONFIG["model_path"]) # Frequent saves
